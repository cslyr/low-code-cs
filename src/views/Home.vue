...
export default {
    ...
    //             // 当对象内部的任何属性变化时，这里的代码都会执行
    //             console.log("curComponent变化了", newVal)
    //         },
    //         deep: true,
    //     },
    // },
    ...
    methods: {
        ...
        handleDrop(e) {
            console.log("触发放置事件",e)
            e.preventDefault() // 表示的是阻止将组件放置到其他组件上
            e.stopPropagation() // 阻止事件冒泡
            const index = e.dataTransfer.getData('index')  // 获取在dragStart当中传递的数据
            const rectInfo = this.editor.getBoundingClientRect() //rectInfo 编辑器画布当中的信息
            if (index) {
                // console.log("index",index)
                console.log("componentList",componentList)
                const component = deepCopy(componentList[index]) // 深拷贝的方式赋值一个组件对象
                component.style.top = e.clientY - rectInfo.y // 组件的位置信息=鼠标的位置信息 - 编辑器当中画布的位置信息
                component.style.left = e.clientX - rectInfo.x
                component.id = generateID() // 生成一个唯一的ID标识信息
                // 根据画面比例修改组件样式比例
                changeComponentSizeWithScale(component)
                this.$store.commit('addComponent', { component }) //在vuex当中添加组件信息
                this.$store.commit('recordSnapshot') // 记录当前的快照
            }
        },

        handleDragOver(e) {
            // console.log('1111')
            e.preventDefault()// 用于解除浏览器不允许将元素放置到其他元素上
            e.dataTransfer.dropEffect = 'copy'  // 放置的操作将会复制被拖拽的元素
        },
