...
function calculateLeftTop(style, curPositon, proportion, needLockProportion, pointInfo) {
    const { symmetricPoint } = pointInfo// 对称点信息，用于计算新的中心点
    // console.log('needLockProportion',needLockProportion)
    /*
        curPositon表示的是移动后的绝对定位
        symmetricPoint对称点信息
        newCenterPoint 计算新的对称点信息
    */
    let newCenterPoint = getCenterPoint(curPositon, symmetricPoint)
    /*
        这个函数用于计算出新的左上角坐标（将旋转的角度也考虑在内，由于Math.atan2是逆时针旋转但是rotate是顺时针所以要加上负号）
        现在知道了鼠标的坐标，知道了旋转的角度，知道了中心点的位置，就能以中心点作为原点，根据二维旋转变换求出来旋转之前div元素的坐标
    */
    let newTopLeftPoint = calculateRotatedPointCoordinate(curPositon, newCenterPoint, -style.rotate)
    // console.log("原来",newTopLeftPoint)
    ...
    */
    ...
    // console.log("newBottomRightPoint",newBottomRightPoint)
